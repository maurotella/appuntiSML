...
\subsection{\textit{Label set} \texorpdfstring{$\Y$}{Y}}
...
\subsection{\textit{Loss function} \texorpdfstring{$\loss$}{l}}
...
\subsection{\textit{Data domain} \texorpdfstring{$\X$}{X}}
...
\subsection{Predittori \texorpdfstring{$\pred$}{f}}
...
funzione $\pred$ è definita da un certo numero di parametri in dato modello}. Un
...
\subsection{\textit{Test set} e \textit{test error}}
...
\subsection{\textit{Learning algorithm}}
Si definisce \textit{training set} un insieme di esempi:
$$ \{(x_1,y_1),\dots,(x_m,y_m)\} $$
che viene usato dal \textbf{\textit{learning algorithm}} per produrre un predittore.
Informalmente, il \textit{learning algorithm} \quotes{impara} dal \textit{training set}.

%$$ S \rightarrow \boxed{A} \rightarrow A(S) $$
